  ...
  <ElContainer class="role tu-container">
    ...
    <ElMain class="tu-main">
      <ElTable :data="tableData.content">
        ...
        <ElTableColumn prop="parentId" label="父模块" align="center"/>
    ...
    <SettingDialog :visible.sync="editDialogVisible" :formData.sync="moduleInfo" @post:module="saveModule"/>
  </ElContainer>
</template>
...
export default {
  async asyncData ({ app }) {
    let tableData = await Module.find()
    return {
      tableData
    }
  },
  ...
  methods: {
    selectProject (row) {
      Object.assign(this.moduleInfo, row)
    },
    async loadData (params = this.params) {
      this.tableData = await Module.find(params)
    },
    async saveModule () {
      await Module.save(this.moduleInfo).then(res => {
        this.loadData()
